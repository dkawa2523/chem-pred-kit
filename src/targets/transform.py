...
class TargetTransform:
    ...
    def _apply_inverse_step(self, y: np.ndarray, step: Dict[str, Any]) -> np.ndarray:
        ...
        if name == "log":
            offset = step.get("state", {}).get("offset", step.get("params", {}).get("offset", step.get("params", {}).get("eps", 0.0)))
            offset = float(offset or 0.0)
            max_log = step.get("params", {}).get("max_log")
            if max_log is None:
                max_log = float(np.log(np.finfo(y.dtype).max))
            y_clip = np.clip(y, a_min=None, a_max=float(max_log))
            return np.exp(y_clip) - offset
        if name == "log1p":
            max_log = step.get("params", {}).get("max_log")
            if max_log is None:
                max_log = float(np.log(np.finfo(y.dtype).max))
            y_clip = np.clip(y, a_min=None, a_max=float(max_log))
            return np.expm1(y_clip)
        if name in {"zscore", "standardize"}:
            mean = step.get("state", {}).get("mean")
            std = step.get("state", {}).get("std")
            if mean is None or std is None:
...
def inverse_target_transforms(
    ...
    for idx, name in enumerate(target_names):
        ...
        valid = np.isfinite(y_arr[:, idx])
        if not np.any(valid):
            continue
        out[valid, idx] = transform.inverse_transform(y_arr[valid, idx])
    if y_was_1d:
        return out.reshape(-1)
    return out
    ...
    if mask is None:
        ...
        if mask_arr.shape != y_arr.shape:
            try:
                ...
                raise ValueError("mask shape mismatch for safe_inverse_transform") from exc
        valid = mask_arr & np.isfinite(y_arr)
    out = np.full_like(y_arr, np.nan, dtype=float)
    if np.any(valid):
        out[valid] = transform.inverse_transform(y_arr[valid])
    return out
