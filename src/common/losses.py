...
def _sanitize_and_combine_mask(
    pred_t: "torch.Tensor",
    ...
    if not finite.all():
        pred_t = torch.where(finite, pred_t, torch.zeros_like(pred_t))
        target_t = torch.where(finite, target_t, torch.zeros_like(target_t))
    if mask is None:
        combined = finite
    else:
        mask_t = mask
        if mask_t.dim() == 1:
            mask_t = mask_t.view(-1, 1)
        combined = mask_t.to(dtype=torch.bool, device=pred_t.device) & finite
    return pred_t, target_t, combined
...
def masked_regression_loss(
    ...
    pred_t, target_t, mask = _sanitize_and_combine_mask(pred_t, target_t, mask)
