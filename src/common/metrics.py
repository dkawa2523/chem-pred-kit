...
def regression_metrics(y_true, y_pred, mask: Any = None) -> Dict[str, float]:
    ...
    if mask is not None:
        ...
        if mask_arr.shape != y_true.shape:
            try:
                mask_arr = np.broadcast_to(mask_arr, y_true.shape)
            except ValueError as exc:
                raise ValueError("mask shape mismatch for regression_metrics") from exc
        mask_arr = mask_arr & np.isfinite(y_true) & np.isfinite(y_pred)
    else:
        mask_arr = np.isfinite(y_true) & np.isfinite(y_pred)
    if mask_arr.ndim != 1:
    ...
    if not np.any(mask_arr):
        return {}
    y_true = y_true[mask_arr]
    y_pred = y_pred[mask_arr]
...
def uncertainty_metrics(
    ...
    if mask is not None:
        ...
        mask_arr = mask_arr & np.isfinite(yt) & np.isfinite(yp)
    else:
        mask_arr = np.isfinite(yt) & np.isfinite(yp)
