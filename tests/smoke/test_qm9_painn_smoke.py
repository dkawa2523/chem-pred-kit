...
def qm9_painn_smoke_artifacts(tmp_path_factory: pytest.TempPathFactory) -> Dict[str, Any]:
    ...
    try:
        from torch_geometric.nn.models import PaiNN as _PaiNN
    except Exception:
        try:
            from torch_geometric.nn import PaiNN as _PaiNN
        except Exception:
            _PaiNN = None
    if _PaiNN is None:
        pytest.skip("PaiNN is unavailable in torch_geometric.")

    from tests.fixtures.qm9_dummy import DummyQM9Dataset
    from tests.fixtures.qm9_helpers import (
        build_qm9_dataset_cfg,
        build_qm9_gnn_painn_eval_cfg,
        build_qm9_gnn_painn_train_cfg,
    )
    ...
    root = tmp_path_factory.mktemp("qm9_painn_smoke")
    ...
    dataset_run_dir = root / "runs" / "build_dataset" / "qm9_painn_smoke"
    ...
    try:
        ...
        logger = get_logger("qm9_painn_dataset", log_file=dataset_run_dir / "build_dataset.log")
        ...
        train_run_root = root / "runs" / "train" / "qm9_painn"
        train_cfg = build_qm9_gnn_painn_train_cfg(train_run_root, dataset_csv, indices_dir, sdf_dir)
        train_run_dir = gnn_train.run(train_cfg)

        eval_run_root = root / "runs" / "evaluate" / "qm9_painn"
        eval_cfg = build_qm9_gnn_painn_eval_cfg(eval_run_root, train_run_dir)
...
def test_qm9_painn_smoke_artifacts(qm9_painn_smoke_artifacts) -> None:
    from src.utils.artifacts import validate_evaluate_artifacts, validate_train_artifacts

    validate_train_artifacts(qm9_painn_smoke_artifacts["train_run_dir"])
    validate_evaluate_artifacts(qm9_painn_smoke_artifacts["eval_run_dir"])
